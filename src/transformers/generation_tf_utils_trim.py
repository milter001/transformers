...
class TFGenerationMixin:
    ...
    def generate(
        ...
        if num_return_sequences > 1 or num_beams > 1:
            # 这里对input_ids做的变换操作值得学习，目标是复制tensor，
            # 但是通过 expand_dims broadcast_to reshape 三个连续操作巧妙实现
        ...
        if self.config.is_encoder_decoder:
            ...
            # 对于encoder_decoder架构，只需要encoder_outputs + start tokens
            input_ids = (
                tf.ones(
            ...
            # 因为获得encoder_output时，input_ids并没有进行复制。为了和复制后的input_ids对齐，这里使用的技巧是repeat batch id 然后gather
        ...
        if num_beams > 1:
            output = self._generate_beam_search(
                ...
                batch_size=effective_batch_size, # 这里的effective_batch_size是原始的batch_size * num_return_sequences
